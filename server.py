...
class WorkBriefHandler(SimpleHTTPRequestHandler):
    ...
    def build_weekly_prompt(self, payload):
        ...
        return f"""你是一名擅长研发周报写作的中文助手。请根据 Git diff 生成一份适合提交给直属上级的中文研发周报。

要求：
1. 只基于 diff 总结，不要编造不存在的功能、数字或业务结果。
2. 不要逐行解释代码，按业务价值、功能变更、缺陷修复、技术优化、风险与下周计划归纳。
3. 语言真实克制，适合职场汇报。
4. 结构包含：本周工作总结、重点成果、技术改进、问题与风险、下周计划。
5. 如果 diff 信息不足，请明确说明只能基于代码变更初步归纳。
